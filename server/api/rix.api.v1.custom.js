...
customFunctions.getLastBlocks = (rix, elements, callback) => {
	let resultArr = [];
	rix.getInfo({})
	   	.then(result => { 
	   		...
	   		async.each(elements, (elem, cb) => {
	   			rix.getBlock({ block_num_or_id: result.head_block_num - elem })
	   				.then(block => {
...
function getBlockOffset(){
	  rix.getBlock({ block_num_or_id: result.head_block_num })
	     .then(block => {
