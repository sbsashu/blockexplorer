...
module.exports = (io, mongoMain, metrics) => {
  ...
  function getDataSocket(){
      ...
      async.parallel({
        ...
        blocks: cb => {
          customFunctions.getLastBlocks(global.rix, blocks, (err, result) => {
                      if (err){
                          log.error(err);
                          return cb('No result');
                      }
        ...
        stat: cb => {
          ...
          if (err){
             ...
             //if (date > SOCKET_HANGUP_TIME){
                 ...
                 config.rixConfig.httpEndpoint = config.endpoints[changeAPI];
                 global.rix = RIX(config.rixConfig);
                 console.error('\x1b[33m%s\x1b[0m', `Change API to [${config.rixConfig.httpEndpoint}], socket error - ${err}`);
  ...
  function getTPS(){
      let timeRequestStart = +new Date(); 
      customFunctions.getLastBlocks(rix, [1, 2], (err, result) => {
            if (err){
                console.error(err);
