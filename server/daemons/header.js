...
config.rixConfig.httpEndpoint = (config.CRON) ? config.CRON_API : config.rixConfig.httpEndpoint;
const rix     		= RIX(config.rixConfig);
...
module.exports = (loggerFileName) => {
	...
	const mongoMain = mongoose.createConnection(config.MONGO_URI, config.MONGO_OPTIONS,
	 (err) => {
	    ...
	    log.info(`[Connected to Mongo RIX in (${loggerFileName}) Daemon]`);
	});
	...
	return {rix, SETTINGS_DB, STATS_ACCOUNT_DB, TABLE_DB, log, logSlack, asyncjs, config, request, req, path, fs};
};