/*
	max TPS, APS daemons
*/ 
const { rix, SETTINGS_DB, STATS_ACCOUNT_DB, log, config } = require('./header')('tps');
...
async function getMaxTps(){
	...
	let info = await wrapper.toStrong(rix.getInfo({}));
...
async function getBlockRecursive(settings, info, elements){
	...
	let [err, block] = await wrapper.to(rix.getBlock({ block_num_or_id: blockNumber }));
	if (err){
